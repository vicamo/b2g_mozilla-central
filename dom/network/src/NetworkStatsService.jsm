...
this.NetworkStatsService = {
  ...
  clearDB: function(aMsgTarget, aMsgJson) {
    ...
    this._db.getAvailableNetworks(function(aError, aNetworksArray) {
      ...
      let networks = aNetworksArray;
      networks.forEach(function(aNetwork, aIndex) {
        networks[aIndex] = {
          network: aNetwork,
          networkId: self._getNetworkId(aNetwork.id, aNetwork.type)
        };
      }, self);
      ...
      self._updateAllStats(function(aResult, aMessage) {
        ...
        self._db.clearStats(networks, function(aError, aUndefined) {
          networks.forEach(function(aNetwork) {
            self._updateCurrentAlarm(aNetwork.networkId);
          }, self);
          aMsgTarget.sendAsyncMessage("NetworkStats:ClearAll:Return", {
            id: aMsgJson.id,
            error: aError,

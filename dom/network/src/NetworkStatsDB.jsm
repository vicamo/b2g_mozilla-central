...
NetworkStatsDB.prototype = {
  ...
  upgradeSchema: function(aTransaction, aDb, aOldVersion, aNewVersion) {
    ...
    (function step(aNextVersion) {
      ...
      func.call(this, aTransaction, aDb, step.bind(this, aNextVersion + 1));
    }).call(this, 1);
  },
  ...
  clearInterfaceStats: function(aNetwork, aResultCb) {
    ...
    this._dbNewTxn(NETWORK_STORE_NAME, "readwrite",
                   function(aTransaction, aNetworkStore) {
      let sample = null;
      let request = aNetworkStore.index("network").openCursor(network, "prev");
      request.onsuccess = function(event) {
        let cursor = event.target.result;
        if (cursor) {
  ...
  _getCurrentStatsFromDate: function(aNetwork, aDate, aResultCb) {
    ...
    this._dbNewTxn(NETWORK_STORE_NAME, "readonly",
                   function(aTransaction, aNetworkStore) {
      let request = null;
      let startTimestamp = this.normalizeDate(aDate);

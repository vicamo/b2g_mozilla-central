                              ...
                              uint32_t& aActualServiceId) const
{
  ...
  if (aOptionalServiceId.WasPassed()) {
    mService->GetItemByServiceId(aOptionalServiceId.Value(),
                                 getter_AddRefs(item));
  } else {
    mService->GetDefaultItem(getter_AddRefs(item));
    if (item) {
      NS_ENSURE_SUCCESS(item->GetServiceId(&aActualServiceId), nullptr);
      MOZ_ASSERT(aActualServiceId <= mStatuses.Length());
    }
  }
                             ...
                             nsIVoicemail* aVoicemail)
{
  ...
  MOZ_ASSERT(aVoicemail);

  nsRefPtr<VoicemailStatus> res = mStatuses[aServiceId];
  if (!res) {
    mStatuses[aServiceId] = res = new VoicemailStatus(GetOwner(), aVoicemail);
  }
                     ...
                     ErrorResult& aRv)
{
  ...
  nsCOMPtr<nsIVoicemail> item = GetItemByServiceId(aServiceId, serviceId);
  if (!item) {
    aRv.Throw(NS_ERROR_UNEXPECTED);
    return nullptr;
  }

  return GetOrCreateStatus(serviceId, item);
}
                          ...
                          ErrorResult& aRv) const
{
  ...
  aRv = item->GetDisplayName(aDisplayName);
}

// nsIVoicemailListener

NS_IMETHODIMP
Voicemail::NotifyInfoChanged(nsIVoicemail* aVoicemail)
{
  // Ignored.
  return NS_OK;
}
...
NS_IMETHODIMP
Voicemail::NotifyStatusChanged(nsIVoicemail* aVoicemail)
{
  NS_ENSURE_ARG_POINTER(aVoicemail);

  uint32_t serviceId = 0;
  if (NS_FAILED(aVoicemail->GetServiceId(&serviceId))) {
  ...
  init.mStatus = GetOrCreateStatus(serviceId, aVoicemail);

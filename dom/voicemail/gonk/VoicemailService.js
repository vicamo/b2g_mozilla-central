...
function Voicemail(aServiceId) {
  this.serviceId = aServiceId;
}
Voicemail.prototype = {
  QueryInterface: XPCOMUtils.generateQI([Ci.nsIVoicemail]),
...
function VoicemailService() {
  ...
  for (let i = 0; i < numItems; i++) {
    this._items.push(new Voicemail(i));
  }

  this._listeners = [];

  // Must be initialized after |this._items|.
...
VoicemailService.prototype = {
  ...
  getItemByServiceId: function(aServiceId) {
    let item = this._items[aServiceId];
    if (!item) {
  ...
  notifyStatusChanged: function(aServiceId, aHasMessages, aMessageCount,
    ...
    let item = this.getItemByServiceId(aServiceId);

    let changed = false;
    if (item.hasMessages != aHasMessages) {
      item.hasMessages = aHasMessages;
      changed = true;
    }
    if (item.messageCount != aMessageCount) {
      item.messageCount = aMessageCount;
      changed = true;
    } else if (aMessageCount == -1) {
      // For MWI using DCS the message count is not available
      changed = true;
    }
    if (item.returnNumber != aReturnNumber) {
      item.returnNumber = aReturnNumber;
      changed = true;
    }
    if (item.returnMessage != aReturnMessage) {
      item.returnMessage = aReturnMessage;
      changed = true;
    }

    if (changed) {
      this._notifyListeners("notifyStatusChanged", item);
    }
  },
  ...
  notifyInfoChanged: function(aServiceId, aNumber, aDisplayName) {
    ...
    if (item.displayName != aDisplayName) {
      item.displayName = aDisplayName;
      changed = true;
    }

    if (changed) {
      this._notifyListeners("notifyInfoChanged", item);
    }
  },
